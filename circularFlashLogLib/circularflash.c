...
static int32_t calculateErasedSpace(void) {
	if (LogFlashTailPtr == 0 && LogFlashHeadPtr == 0) {
		...
		return FLASH_LOGS_LENGTH - (LogFlashHeadPtr + (FLASH_LOGS_LENGTH - LogFlashTailPtr));
	}
	else {
		FLASH_DEBUG("FLASH: Log corrupted\r\n");
		return 0;
...
static uint32_t circFlashInsertWrite(uint32_t FlashAddress, unsigned char * buff, uint32_t len) {
...
int32_t circularReadLogPartial(unsigned char * buff, int32_t seek, int32_t desiredlen, int32_t * remaining) {
	...
	if (space > 0 && desiredlen > 0) {
		if (LogFlashHeadPtr > LogFlashTailPtr) {
			res = circFlashRead(FLASH_LOGS_ADDRESS + LogFlashTailPtr + seek, buff, desiredlen);
			if (res != desiredlen) {
			...
			*remaining = (space - seek - desiredlen);
			...
			if (seek > firstlen) {
				...
				res = circFlashRead(FLASH_LOGS_ADDRESS + (seek - firstlen), buff, desiredlen);
				if (res != desiredlen) {
					FLASH_DEBUG("FLASH: IO error\r\n");
					ret = 0;
					*remaining = 0;
				...
				*remaining = (space - seek - desiredlen);
				...
				if (seek + desiredlen + LogFlashTailPtr > FLASH_LOGS_LENGTH) {
					...
					res = circFlashRead(FLASH_LOGS_ADDRESS + LogFlashTailPtr + seek, buff, secondlen);
					if (res != secondlen) {
						FLASH_DEBUG("FLASH: IO error\r\n");
						ret = 0;
						*remaining = 0;
					...
					res = circFlashRead(FLASH_LOGS_ADDRESS, &buff[secondlen], desiredlen - secondlen);
...
int32_t circularClearLog(void) {
	FLASH_MUTEX_ENTER();
	if (circFlashErase(FLASH_LOGS_ADDRESS, FLASH_LOGS_LENGTH) != FLASH_LOGS_LENGTH) {
...
int circularWriteLog(unsigned char * buf, int len) {
	...
	if (EraseSpace == 0) {
		...
		if (circFlashErase(FLASH_LOGS_ADDRESS + LogFlashTailPtr, FLASH_SECTOR_SIZE) != FLASH_SECTOR_SIZE) {
			FLASH_DEBUG("FLASH: Erase IO error\r\n");
			goto badexit;
		}
	...
	//Does it wrap?
	//The FLASH_WRITE_SIZE boundary will assume that writing FLASH_ERASED will leave existing alone
	if (LogFlashHeadPtr + len > FLASH_LOGS_LENGTH) {
		//Wrapped
		firstlen = FLASH_LOGS_LENGTH - LogFlashHeadPtr;
		res = circFlashInsertWrite(FLASH_LOGS_ADDRESS + LogFlashHeadPtr, buf, firstlen);
		if (res != firstlen) {
			FLASH_DEBUG("FLASH: Write IO error\r\n");
			goto badexit;
		}
		res = circFlashInsertWrite(FLASH_LOGS_ADDRESS, &buf[firstlen], len - firstlen);
		if (res != len - firstlen) {
			FLASH_DEBUG("FLASH: Write IO error\r\n");
			goto badexit;
		}
